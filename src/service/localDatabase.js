...
class LocalDatabase {
    ...
    setmaxTableSize(limit) {
        LocalDatabase.maxTableSize = limit;
    }
    ...
    async initUserTables(userId) {
        ...
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_feed_gps (id INTEGER PRIMARY KEY, created_at TEXT, user_id TEXT, display_name TEXT, location TEXT, world_name TEXT, previous_location TEXT, time INTEGER, group_name TEXT)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_feed_status (id INTEGER PRIMARY KEY, created_at TEXT, user_id TEXT, display_name TEXT, status TEXT, status_description TEXT, previous_status TEXT, previous_status_description TEXT)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_feed_bio (id INTEGER PRIMARY KEY, created_at TEXT, user_id TEXT, display_name TEXT, bio TEXT, previous_bio TEXT)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_feed_avatar (id INTEGER PRIMARY KEY, created_at TEXT, user_id TEXT, display_name TEXT, owner_id TEXT, avatar_name TEXT, current_avatar_image_url TEXT, current_avatar_thumbnail_image_url TEXT, previous_current_avatar_image_url TEXT, previous_current_avatar_thumbnail_image_url TEXT)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_feed_online_offline (id INTEGER PRIMARY KEY, created_at TEXT, user_id TEXT, display_name TEXT, type TEXT, location TEXT, world_name TEXT, time INTEGER, group_name TEXT)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_friend_log_current (user_id TEXT PRIMARY KEY, display_name TEXT, trust_level TEXT, friend_number INTEGER)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_friend_log_history (id INTEGER PRIMARY KEY, created_at TEXT, type TEXT, user_id TEXT, display_name TEXT, previous_display_name TEXT, trust_level TEXT, previous_trust_level TEXT, friend_number INTEGER)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_notifications (id TEXT PRIMARY KEY, created_at TEXT, type TEXT, sender_user_id TEXT, sender_username TEXT, receiver_user_id TEXT, message TEXT, world_id TEXT, world_name TEXT, image_url TEXT, invite_message TEXT, request_message TEXT, response_message TEXT, expired INTEGER)`
        );
        await sqliteService.executeNonQuery(
            `CREATE TABLE IF NOT EXISTS ${LocalDatabase.userPrefix}_moderation (user_id TEXT PRIMARY KEY, updated_at TEXT, display_name TEXT, block INTEGER, mute INTEGER)`
        );
        await sqliteService.executeNonQuery(
    ...
    async getFeedDatabase() {
        ...
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_gps WHERE created_at >= date('${dateOffset}') ORDER BY id DESC`);
        await sqliteService.execute((dbRow) => {
            var row = {
                rowId: dbRow[0],
                created_at: dbRow[1],
            ...
            feedDatabase.unshift(row);
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_status WHERE created_at >= date('${dateOffset}') ORDER BY id DESC`);
        await sqliteService.execute((dbRow) => {
            var row = {
                rowId: dbRow[0],
                created_at: dbRow[1],
            ...
            feedDatabase.unshift(row);
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_bio WHERE created_at >= date('${dateOffset}') ORDER BY id DESC`);
        await sqliteService.execute((dbRow) => {
            var row = {
                rowId: dbRow[0],
                created_at: dbRow[1],
            ...
            feedDatabase.unshift(row);
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_avatar WHERE created_at >= date('${dateOffset}') ORDER BY id DESC`);
        await sqliteService.execute((dbRow) => {
            var row = {
                rowId: dbRow[0],
                created_at: dbRow[1],
            ...
            feedDatabase.unshift(row);
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_online_offline WHERE created_at >= date('${dateOffset}') ORDER BY id DESC`);
    ...
    async getFriendLogCurrent() {
        ...
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_friend_log_current`);
        return friendLogCurrent;
    }

    setFriendLogCurrent(entry) {
        sqliteService.executeNonQuery(
            `INSERT OR REPLACE INTO ${LocalDatabase.userPrefix}_friend_log_current (user_id, display_name, trust_level, friend_number) VALUES (@user_id, @display_name, @trust_level, @friend_number)`,
            {
                '@user_id': entry.userId,
    ...
    setFriendLogCurrentArray(inputData) {
        ...
        sqliteService.executeNonQuery(
            `INSERT OR REPLACE INTO ${LocalDatabase.userPrefix}_friend_log_current (user_id, display_name, trust_level, friend_number) VALUES ${sqlValues}`
        );
    }

    deleteFriendLogCurrent(userId) {
        sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_friend_log_current WHERE user_id = @user_id`,
            {
                '@user_id': userId
            }
        );
    ...
    async getMaxFriendLogNumber() {
        ...
        }, `SELECT MAX(friend_number) FROM ${LocalDatabase.userPrefix}_friend_log_current`);
        return friendNumber;
    }
    ...
    async getFriendLogHistory() {
        ...
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_friend_log_history`);
        return friendLogHistory;
    }

    addFriendLogHistory(entry) {
        sqliteService.executeNonQuery(
            `INSERT OR IGNORE INTO ${LocalDatabase.userPrefix}_friend_log_history (created_at, type, user_id, display_name, previous_display_name, trust_level, previous_trust_level, friend_number) VALUES (@created_at, @type, @user_id, @display_name, @previous_display_name, @trust_level, @previous_trust_level, @friend_number)`,
            {
    ...
    addFriendLogHistoryArray(inputData) {
        ...
        sqliteService.executeNonQuery(
            `INSERT OR IGNORE INTO ${LocalDatabase.userPrefix}_friend_log_history (created_at, type, user_id, display_name, previous_display_name, trust_level, previous_trust_level, friend_number) VALUES ${sqlValues}`
        );
    }

    deleteFriendLogHistory(rowId) {
        sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_friend_log_history WHERE id = @row_id`,
            {
                '@row_id': rowId
            }
        );
    ...
    addGPSToDatabase(entry) {
        sqliteService.executeNonQuery(
            `INSERT OR IGNORE INTO ${LocalDatabase.userPrefix}_feed_gps (created_at, user_id, display_name, location, world_name, previous_location, time, group_name) VALUES (@created_at, @user_id, @display_name, @location, @world_name, @previous_location, @time, @group_name)`,
            {
                '@created_at': entry.created_at,
                '@user_id': entry.userId,
                '@display_name': entry.displayName,
    ...
    addStatusToDatabase(entry) {
        sqliteService.executeNonQuery(
            `INSERT OR IGNORE INTO ${LocalDatabase.userPrefix}_feed_status (created_at, user_id, display_name, status, status_description, previous_status, previous_status_description) VALUES (@created_at, @user_id, @display_name, @status, @status_description, @previous_status, @previous_status_description)`,
            {
                '@created_at': entry.created_at,
                '@user_id': entry.userId,
                '@display_name': entry.displayName,
    ...
    addBioToDatabase(entry) {
        sqliteService.executeNonQuery(
            `INSERT OR IGNORE INTO ${LocalDatabase.userPrefix}_feed_bio (created_at, user_id, display_name, bio, previous_bio) VALUES (@created_at, @user_id, @display_name, @bio, @previous_bio)`,
            {
                '@created_at': entry.created_at,
                '@user_id': entry.userId,
                '@display_name': entry.displayName,
    ...
    addAvatarToDatabase(entry) {
        sqliteService.executeNonQuery(
            `INSERT OR IGNORE INTO ${LocalDatabase.userPrefix}_feed_avatar (created_at, user_id, display_name, owner_id, avatar_name, current_avatar_image_url, current_avatar_thumbnail_image_url, previous_current_avatar_image_url, previous_current_avatar_thumbnail_image_url) VALUES (@created_at, @user_id, @display_name, @owner_id, @avatar_name, @current_avatar_image_url, @current_avatar_thumbnail_image_url, @previous_current_avatar_image_url, @previous_current_avatar_thumbnail_image_url)`,
            {
                '@created_at': entry.created_at,
                '@user_id': entry.userId,
                '@display_name': entry.displayName,
    ...
    addOnlineOfflineToDatabase(entry) {
        sqliteService.executeNonQuery(
            `INSERT OR IGNORE INTO ${LocalDatabase.userPrefix}_feed_online_offline (created_at, user_id, display_name, type, location, world_name, time, group_name) VALUES (@created_at, @user_id, @display_name, @type, @location, @world_name, @time, @group_name)`,
            {
                '@created_at': entry.created_at,
                '@user_id': entry.userId,
                '@display_name': entry.displayName,
    ...
    async getNotifications() {
        ...
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_notifications ORDER BY created_at DESC LIMIT ${LocalDatabase.maxTableSize}`);
    ...
    deleteNotification(rowId) {
        sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_notifications WHERE id = @row_id`,
            {
                '@row_id': rowId
            }
        );
    ...
    updateNotificationExpired(entry) {
        ...
        sqliteService.executeNonQuery(
            `UPDATE ${LocalDatabase.userPrefix}_notifications SET expired = @expired WHERE id = @id`,
            {
                '@id': entry.id,
                '@expired': expired
            }
    ...
    async getGpsTableSize() {
        ...
        await sqliteService.execute((row) => {
            size = row[0];
        }, `SELECT COUNT(*) FROM ${LocalDatabase.userPrefix}_feed_gps`);
        return size;
    }
    ...
    async getStatusTableSize() {
        ...
        await sqliteService.execute((row) => {
            size = row[0];
        }, `SELECT COUNT(*) FROM ${LocalDatabase.userPrefix}_feed_status`);
        return size;
    }
    ...
    async getBioTableSize() {
        ...
        await sqliteService.execute((row) => {
            size = row[0];
        }, `SELECT COUNT(*) FROM ${LocalDatabase.userPrefix}_feed_bio`);
        return size;
    }
    ...
    async getAvatarTableSize() {
        ...
        await sqliteService.execute((row) => {
            size = row[0];
        }, `SELECT COUNT(*) FROM ${LocalDatabase.userPrefix}_feed_avatar`);
        return size;
    }
    ...
    async getOnlineOfflineTableSize() {
        ...
        await sqliteService.execute((row) => {
            size = row[0];
        }, `SELECT COUNT(*) FROM ${LocalDatabase.userPrefix}_feed_online_offline`);
        return size;
    }
    ...
    async getFriendLogHistoryTableSize() {
        ...
        await sqliteService.execute((row) => {
            size = row[0];
        }, `SELECT COUNT(*) FROM ${LocalDatabase.userPrefix}_friend_log_history`);
        return size;
    }
    ...
    async getNotificationTableSize() {
        ...
        await sqliteService.execute((row) => {
            size = row[0];
        }, `SELECT COUNT(*) FROM ${LocalDatabase.userPrefix}_notifications`);
        return size;
    }
    ...
    static async getFeedByInstanceId(instanceId, filters, vipList) {
        ...
        if (gps) {
            await sqliteService.execute((dbRow) => {
                ...
                feedDatabase.unshift(row);
            }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_gps WHERE location LIKE '%${instanceId}%' ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (online || offline) {
            var query = '';
            if (!online || !offline) {
            ...
            await sqliteService.execute((dbRow) => {
                ...
                feedDatabase.unshift(row);
            }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_online_offline WHERE (location LIKE '%${instanceId}%' ${query}) ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        var compareByCreatedAt = function (a, b) {
            var A = a.created_at;
            var B = b.created_at;
    ...
    async lookupFeedDatabase(search, filters, vipList) {
        ...
        if (search.startsWith('wrld_')) {
            return LocalDatabase.getFeedByInstanceId(search, filters, vipList);
        }
        var vipQuery = '';
        if (vipList.length > 0) {
            vipQuery = 'AND user_id IN (';
        ...
        if (gps) {
            await sqliteService.execute((dbRow) => {
                ...
                feedDatabase.unshift(row);
            }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_gps WHERE (display_name LIKE '%${search}%' OR world_name LIKE '%${search}%') ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (status) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                feedDatabase.unshift(row);
            }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_status WHERE (display_name LIKE '%${search}%' OR status LIKE '%${search}%' OR status_description LIKE '%${search}%') ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (bio) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                feedDatabase.unshift(row);
            }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_bio WHERE (display_name LIKE '%${search}%' OR bio LIKE '%${search}%') ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (avatar) {
            var query = '';
            if (aviPrivate) {
            ...
            await sqliteService.execute((dbRow) => {
                ...
                feedDatabase.unshift(row);
            }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_avatar WHERE ((display_name LIKE '%${search}%' OR avatar_name LIKE '%${search}%') ${query}) ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (online || offline) {
            var query = '';
            if (!online || !offline) {
            ...
            await sqliteService.execute((dbRow) => {
                ...
                feedDatabase.unshift(row);
            }, `SELECT * FROM ${LocalDatabase.userPrefix}_feed_online_offline WHERE ((display_name LIKE '%${search}%' OR world_name LIKE '%${search}%') ${query}) ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        var compareByCreatedAt = function (a, b) {
            var A = a.created_at;
            var B = b.created_at;
    ...
    static async getGameLogByLocation(instanceId, filters) {
        ...
        if (location) {
            await sqliteService.execute((dbRow) => {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_location WHERE location LIKE '%${instanceId}%' ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (onplayerjoined || onplayerleft) {
            ...
            await sqliteService.execute((dbRow) => {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_join_leave WHERE (location LIKE '%${instanceId}%' AND user_id != '${LocalDatabase.userId}') ${query} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (portalspawn) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_portal_spawn WHERE location LIKE '%${instanceId}%' ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (videoplay) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_video_play WHERE location LIKE '%${instanceId}%' ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (resourceload_string || resourceload_image) {
            ...
            await sqliteService.execute((dbRow) => {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_resource_load WHERE location LIKE '%${instanceId}%' ${checkString} ${checkImage} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        var compareByCreatedAt = function (a, b) {
            var A = a.created_at;
            var B = b.created_at;
        ...
        gamelogDatabase.splice(
            0,
            gamelogDatabase.length - LocalDatabase.maxTableSize
        );
        return gamelogDatabase;
    }
    ...
    async lookupGameLogDatabase(search, filters, vipList = []) {
        var search = search.replaceAll("'", "''");
        if (search.startsWith('wrld_')) {
            return LocalDatabase.getGameLogByLocation(search, filters);
        }
        let vipQuery = '';
        if (vipList.length > 0) {
            vipQuery = 'AND user_id IN (';
        ...
        if (location) {
            await sqliteService.execute((dbRow) => {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_location WHERE world_name LIKE '%${search}%' ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (onplayerjoined || onplayerleft) {
            var query = '';
            if (!onplayerjoined || !onplayerleft) {
            ...
            await sqliteService.execute((dbRow) => {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_join_leave WHERE (display_name LIKE '%${search}%' AND user_id != '${LocalDatabase.userId}') ${vipQuery} ${query} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (portalspawn) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_portal_spawn WHERE (display_name LIKE '%${search}%' OR world_name LIKE '%${search}%') ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (msgevent) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_event WHERE data LIKE '%${search}%' ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (external) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_external WHERE (display_name LIKE '%${search}%' OR message LIKE '%${search}%') ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (videoplay) {
            await sqliteService.execute((dbRow) => {
                var row = {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_video_play WHERE (video_url LIKE '%${search}%' OR video_name LIKE '%${search}%' OR display_name LIKE '%${search}%') ${vipQuery} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        if (resourceload_string || resourceload_image) {
            var checkString = '';
            var checkImage = '';
            ...
            await sqliteService.execute((dbRow) => {
                ...
                gamelogDatabase.unshift(row);
            }, `SELECT * FROM gamelog_resource_load WHERE resource_url LIKE '%${search}%' ${checkString} ${checkImage} ORDER BY id DESC LIMIT ${LocalDatabase.maxTableSize}`);
        }
        var compareByCreatedAt = function (a, b) {
            var A = a.created_at;
            var B = b.created_at;
    ...
    async getModeration(userId) {
        ...
        await sqliteService.execute(
            ...
            `SELECT * FROM ${LocalDatabase.userPrefix}_moderation WHERE user_id = @userId`,
            {
                '@userId': userId
            }
        );
    ...
    setModeration(entry) {
        ...
        sqliteService.executeNonQuery(
            `INSERT OR REPLACE INTO ${LocalDatabase.userPrefix}_moderation (user_id, updated_at, display_name, block, mute) VALUES (@user_id, @updated_at, @display_name, @block, @mute)`,
            {
                '@user_id': entry.userId,
    ...
    deleteModeration(userId) {
        sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_moderation WHERE user_id = @user_id`,
            {
                '@user_id': userId
            }
        );
    ...
    async cleanLegendFromFriendLog() {
        await sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_friend_log_history
    ...
    addAvatarToHistory(avatarId) {
        sqliteService.executeNonQuery(
            `INSERT INTO ${LocalDatabase.userPrefix}_avatar_history (avatar_id, created_at, time)
    ...
    async getAvatarTimeSpent(avatarId) {
        ...
        await sqliteService.execute(
            ...
            `SELECT time FROM ${LocalDatabase.userPrefix}_avatar_history WHERE avatar_id = @avatarId`,
            {
                '@avatarId': avatarId
            }
        );
    ...
    addAvatarTimeSpent(avatarId, timeSpent) {
        sqliteService.executeNonQuery(
            `UPDATE ${LocalDatabase.userPrefix}_avatar_history SET time = time + @timeSpent WHERE avatar_id = @avatarId`,
            {
                '@avatarId': avatarId,
                '@timeSpent': timeSpent
            }
    ...
    async getAvatarHistory(currentUserId, limit = 100) {
        ...
        await sqliteService.execute((dbRow) => {
            ...
            data.push(row);
        }, `SELECT * FROM ${LocalDatabase.userPrefix}_avatar_history INNER JOIN cache_avatar ON cache_avatar.id = ${LocalDatabase.userPrefix}_avatar_history.avatar_id WHERE author_id != "${currentUserId}" ORDER BY ${LocalDatabase.userPrefix}_avatar_history.created_at DESC LIMIT ${limit}`);
        return data;
    }
    ...
    clearAvatarHistory() {
        sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_avatar_history`
        );
    ...
    async fixBrokenNotifications() {
        await sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_notifications WHERE (created_at is null or created_at = '')`
        );
    }

    async fixBrokenGroupChange() {
        await sqliteService.executeNonQuery(
            `DELETE FROM ${LocalDatabase.userPrefix}_notifications WHERE type = 'groupChange' AND created_at < '2024-04-23T03:00:00.000Z'`
        );
    }
    ...
    async updateTableForAvatarHistory() {
        await sqliteService.execute((dbRow) => {
            ...
            if (!columnExists) {
                sqliteService.executeNonQuery(
                    `ALTER TABLE ${LocalDatabase.userPrefix}_avatar_history ADD time INTEGER DEFAULT 0`
                );
            }
        }, `PRAGMA table_info(${LocalDatabase.userPrefix}_avatar_history);`);
    }

    async fixCancelFriendRequestTypo() {
        await sqliteService.executeNonQuery(
            `UPDATE ${LocalDatabase.userPrefix}_friend_log_history SET type = 'CancelFriendRequest' WHERE type = 'CancelFriendRequst'`
        );
    }
    ...
    async getInstanceActivity(startDate, endDate) {
        ...
        await sqliteService.execute(
            (row) => {
                ...
                if (rowData.user_id === LocalDatabase.userId) {
