...
describe('extractFileId', () => {
    it('extracts file ID from a URL', () => {
        expect(extractFileId('https://api.vrchat.cloud/file/file_abc123-def/1/file')).toBe('file_abc123-def');
    });

    it('extracts file ID from a plain string', () => {
        expect(extractFileId('file_0123456789abcdef')).toBe('file_0123456789abcdef');
    });
...
describe('extractVariantVersion', () => {
    it('extracts version from query parameter', () => {
        expect(extractVariantVersion('https://example.com/file?v=42')).toBe('42');
    });

    it('returns 0 when no v parameter', () => {
        expect(extractVariantVersion('https://example.com/file?other=1')).toBe('0');
    });
