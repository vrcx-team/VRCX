import { getNotificationMessage, getUserIdFromNoty, toNotificationText } from '../utils/notificationMessage';
...
describe('getNotificationMessage', () => {
    ...
    test('OnPlayerJoined', () => {
        const result = getNotificationMessage({ type: 'OnPlayerJoined', displayName: 'Alice' }, '');
        expect(result).toEqual({ title: 'Alice', body: 'has joined' });
    });

    test('OnPlayerLeft', () => {
        const result = getNotificationMessage({ type: 'OnPlayerLeft', displayName: 'Bob' }, '');
        expect(result).toEqual({ title: 'Bob', body: 'has left' });
    });

    test('OnPlayerJoining', () => {
        const result = getNotificationMessage({ type: 'OnPlayerJoining', displayName: 'Alice' }, '');
    ...
    test('Online without worldName', () => {
        const result = getNotificationMessage({ type: 'Online', displayName: 'Alice', worldName: '' }, '');
    ...
    test('Offline', () => {
        const result = getNotificationMessage({ type: 'Offline', displayName: 'Alice' }, '');
    ...
    test('Friend', () => {
        const result = getNotificationMessage({ type: 'Friend', displayName: 'Dave' }, '');
        expect(result).toEqual({
            title: 'Dave',
    ...
    test('boop', () => {
        const result = getNotificationMessage({ type: 'boop', senderUsername: 'Eve', message: 'boop!' }, '');
        expect(result).toEqual({ title: 'Eve', body: 'boop!' });
    });

    test('groupChange', () => {
        const result = getNotificationMessage({ type: 'groupChange', senderUsername: 'Mod', message: 'rank up' }, '');
        expect(result).toEqual({ title: 'Mod', body: 'rank up' });
    });

    test('group.announcement', () => {
        const result = getNotificationMessage({ type: 'group.announcement', message: 'Hello all' }, '');
        expect(result).toEqual({
    ...
    test('PortalSpawn without displayName', () => {
        const result = getNotificationMessage({ type: 'PortalSpawn', displayName: '' }, '');
        expect(result).toEqual({
            title: '',
    ...
    test('VideoPlay', () => {
        const result = getNotificationMessage({ type: 'VideoPlay', notyName: 'Cool Song' }, '');
        expect(result).toEqual({
            title: 'Now playing',
            body: 'Cool Song'
        });
    ...
    test('BlockedOnPlayerJoined', () => {
        const result = getNotificationMessage({ type: 'BlockedOnPlayerJoined', displayName: 'Troll' }, '');
        expect(result).toEqual({
            title: 'Troll',
    ...
    test('Event', () => {
        const result = getNotificationMessage({ type: 'Event', data: 'something happened' }, '');
        expect(result).toEqual({
            title: 'Event',
    ...
    test('External', () => {
        const result = getNotificationMessage({ type: 'External', message: 'ext msg' }, '');
    ...
    test('AvatarChange', () => {
        const result = getNotificationMessage({ type: 'AvatarChange', displayName: 'Alice', name: 'CoolAvatar' }, '');
        expect(result).toEqual({
            title: 'Alice',
    ...
    test('Blocked', () => {
        const result = getNotificationMessage({ type: 'Blocked', displayName: 'Troll' }, '');
        expect(result).toEqual({ title: 'Troll', body: 'has blocked you' });
    });

    test('Unblocked', () => {
        const result = getNotificationMessage({ type: 'Unblocked', displayName: 'Troll' }, '');
        expect(result).toEqual({
            title: 'Troll',
    ...
    test('Muted', () => {
        const result = getNotificationMessage({ type: 'Muted', displayName: 'Alice' }, '');
        expect(result).toEqual({ title: 'Alice', body: 'has muted you' });
    });

    test('Unmuted', () => {
        const result = getNotificationMessage({ type: 'Unmuted', displayName: 'Alice' }, '');
        expect(result).toEqual({ title: 'Alice', body: 'has unmuted you' });
    });

    test('BlockedOnPlayerLeft', () => {
        const result = getNotificationMessage({ type: 'BlockedOnPlayerLeft', displayName: 'Troll' }, '');
        expect(result).toEqual({
            title: 'Troll',
    ...
    test('MutedOnPlayerJoined', () => {
        const result = getNotificationMessage({ type: 'MutedOnPlayerJoined', displayName: 'MutedUser' }, '');
        expect(result).toEqual({
            title: 'MutedUser',
    ...
    test('MutedOnPlayerLeft', () => {
        const result = getNotificationMessage({ type: 'MutedOnPlayerLeft', displayName: 'MutedUser' }, '');
        expect(result).toEqual({
            title: 'MutedUser',
    ...
    test('group.informative', () => {
        const result = getNotificationMessage({ type: 'group.informative', message: 'Info msg' }, '');
        expect(result).toEqual({
    ...
    test('group.invite', () => {
        const result = getNotificationMessage({ type: 'group.invite', message: 'Join us' }, '');
        expect(result).toEqual({
            title: 'Group Invite',
            body: 'Join us'
        });
    ...
    test('group.joinRequest', () => {
        const result = getNotificationMessage({ type: 'group.joinRequest', message: 'Request' }, '');
        expect(result).toEqual({
    ...
    test('group.transfer', () => {
        const result = getNotificationMessage({ type: 'group.transfer', message: 'Transfer ownership' }, '');
        expect(result).toEqual({
    ...
    test('group.queueReady', () => {
        const result = getNotificationMessage({ type: 'group.queueReady', message: 'Queue is ready' }, '');
        expect(result).toEqual({
    ...
    test('instance.closed', () => {
        const result = getNotificationMessage({ type: 'instance.closed', message: 'Closed' }, '');
        expect(result).toEqual({
...
describe('toNotificationText', () => {
    test('body-only types return just the body', () => {
        ...
        expect(toNotificationText('Group Announcement', 'Hello', 'group.announcement')).toBe('Hello');
        expect(toNotificationText('Event', 'data', 'Event')).toBe('data');
        expect(toNotificationText('External', 'msg', 'External')).toBe('msg');
        expect(toNotificationText('Instance Closed', 'closing', 'instance.closed')).toBe('closing');
    });

    test('colon separator types use ": "', () => {
        expect(toNotificationText('Mod', 'rank up', 'groupChange')).toBe('Mod: rank up');
        expect(toNotificationText('Now playing', 'Song', 'VideoPlay')).toBe('Now playing: Song');
    });

    test('colon separator with empty title returns body only', () => {
        expect(toNotificationText('', 'rank up', 'groupChange')).toBe('rank up');
    });

    test('custom format messages for blocked/muted', () => {
        expect(toNotificationText('Troll', 'blocked user has joined', 'BlockedOnPlayerJoined')).toBe(
            'Blocked user Troll has joined'
        );
        expect(toNotificationText('Troll', 'blocked user has left', 'BlockedOnPlayerLeft')).toBe(
            'Blocked user Troll has left'
        );
        expect(toNotificationText('Troll', 'muted user has joined', 'MutedOnPlayerJoined')).toBe(
            'Muted user Troll has joined'
        );
        expect(toNotificationText('Troll', 'muted user has left', 'MutedOnPlayerLeft')).toBe(
            'Muted user Troll has left'
        );
    });

    test('default types use space separator', () => {
        expect(toNotificationText('Alice', 'has joined', 'OnPlayerJoined')).toBe('Alice has joined');
        expect(toNotificationText('Bob', 'has logged out', 'Offline')).toBe('Bob has logged out');
    });

    test('default with empty title returns body only', () => {
        expect(toNotificationText('', 'User has spawned a portal', 'PortalSpawn')).toBe('User has spawned a portal');
    });
});

describe('getNotificationMessage with displayNameOverride', () => {
    test('overrides displayName in title', () => {
        const result = getNotificationMessage({ type: 'OnPlayerJoined', displayName: 'Alice' }, '', 'NickAlice');
        expect(result).toEqual({ title: 'NickAlice', body: 'has joined' });
    });

    test('overrides senderUsername in sender-based types', () => {
        const result = getNotificationMessage({ type: 'friendRequest', senderUsername: 'Bob' }, '', 'NickBob');
        expect(result).toEqual({
            title: 'NickBob',
            body: 'has sent you a friend request'
        });
    ...
    test('falls back to noty fields when override is empty', () => {
        const result = getNotificationMessage({ type: 'OnPlayerLeft', displayName: 'Alice' }, '', '');
...
describe('getUserIdFromNoty', () => {
    ...
    test('prefers userId over senderUserId', () => {
        expect(getUserIdFromNoty({ userId: 'usr_1', senderUserId: 'usr_2' })).toBe('usr_1');
    });
