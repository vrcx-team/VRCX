<template>
    <safe-dialog ref="groupDialogRef" :visible.sync="groupDialog.visible" :show-close="false" width="770px"
        class="x-dialog x-group-dialog">
        <div class="group-banner-image">
            <el-popover placement="right" width="500px" trigger="click">
                <img slot="reference" v-lazy="groupDialog.ref.bannerUrl"
                    style="flex: none; width: 100%; aspect-ratio: 6/1; object-fit: cover; border-radius: 4px"
                    class="x-link" />
                <img v-lazy="groupDialog.ref.bannerUrl" style="width: 854px; height: 480px" class="x-link"
        ...
        <div v-loading="groupDialog.loading" class="group-body">
            <div style="display: flex">
                <el-popover placement="right" width="500px" trigger="click">
                    <img slot="reference" v-lazy="groupDialog.ref.iconUrl"
                        style="flex: none; width: 120px; height: 120px; border-radius: 12px" class="x-link" />
                    <img v-lazy="groupDialog.ref.iconUrl" style="width: 500px; height: 500px" class="x-link"
                ...
                <div style="flex: 1; display: flex; align-items: center; margin-left: 15px">
                    <div class="group-header" style="flex: 1">
                        ...
                        <el-popover placement="top" trigger="click">
                            <span slot="reference" class="dialog-title" style="margin-right: 5px; cursor: pointer"
                                v-text="groupDialog.ref.name" @click="copyToClipboard(groupDialog.ref.name)"></span>
                            <span style="display: block; text-align: center; font-family: monospace">{{
                                textToHex(groupDialog.ref.name)
                                }}</span>
                        </el-popover>
                        <span class="group-discriminator x-grey"
                        ...
                        <div style="margin-top: 5px">
                            <span class="x-link x-grey" style="font-family: monospace"
                                @click="showUserDialog(groupDialog.ref.ownerId)"
                                v-text="groupDialog.ownerDisplayName"></span>
                        </div>
                        <div class="group-tags">
                            <el-tag v-if="groupDialog.ref.isVerified" type="info" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.verified') }}
                            </el-tag>
                            <el-tag v-if="groupDialog.ref.privacy === 'private'" type="danger" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.private') }}
                            </el-tag>
                            <el-tag v-if="groupDialog.ref.privacy === 'default'" type="success" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.public') }}
                            </el-tag>
                            <el-tag v-if="groupDialog.ref.joinState === 'open'" type="success" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.open') }}
                            </el-tag>
                            <el-tag v-else-if="groupDialog.ref.joinState === 'request'" type="warning" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.request') }}
                            </el-tag>
                            <el-tag v-else-if="groupDialog.ref.joinState === 'invite'" type="danger" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.invite') }}
                            </el-tag>
                            <el-tag v-else-if="groupDialog.ref.joinState === 'closed'" type="danger" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.closed') }}
                            </el-tag>
                            <el-tag v-if="groupDialog.inGroup" type="info" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.joined') }}
                            </el-tag>
                            <el-tag v-if="groupDialog.ref.myMember && groupDialog.ref.myMember.bannedAt" type="danger"
                                effect="plain" size="mini" style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.group.tags.banned') }}
                            </el-tag>
                            <template v-if="groupDialog.inGroup && groupDialog.ref.myMember">
                                <el-tag v-if="groupDialog.ref.myMember.visibility === 'visible'" type="info"
                                    effect="plain" size="mini" style="margin-right: 5px; margin-top: 5px">
                                    {{ t('dialog.group.tags.visible') }}
                                </el-tag>
                                <el-tag v-else-if="groupDialog.ref.myMember.visibility === 'friends'" type="info"
                                    effect="plain" size="mini" style="margin-right: 5px; margin-top: 5px">
                                    {{ t('dialog.group.tags.friends') }}
                                </el-tag>
                                <el-tag v-else-if="groupDialog.ref.myMember.visibility === 'hidden'" type="info"
                                    effect="plain" size="mini" style="margin-right: 5px; margin-top: 5px">
                                    {{ t('dialog.group.tags.hidden') }}
                                </el-tag>
                                <el-tag v-if="groupDialog.ref.myMember.isSubscribedToAnnouncements" type="info"
                                    effect="plain" size="mini" style="margin-right: 5px; margin-top: 5px">
                        ...
                        <div class="group-description" style="margin-top: 5px">
                            <span v-show="groupDialog.ref.name !== groupDialog.ref.description" style="font-size: 12px"
                                v-text="groupDialog.ref.description"></span>
                        </div>
                    </div>
                    <div style="flex: none; margin-left: 10px">
                        <template v-if="groupDialog.inGroup && groupDialog.ref?.myMember">
                            <el-tooltip v-if="groupDialog.ref.myMember?.isRepresenting" placement="top"
                                :content="t('dialog.group.actions.unrepresent_tooltip')" :disabled="hideTooltips">
                                <el-button type="warning" icon="el-icon-star-on" circle style="margin-left: 5px"
                            ...
                            <el-tooltip v-else placement="top" :content="t('dialog.group.actions.represent_tooltip')"
                                ...
                                <span>
                                    <el-button type="default" icon="el-icon-star-off" circle style="margin-left: 5px"
                        ...
                        <template v-else-if="groupDialog.ref.myMember?.membershipStatus === 'requested'">
                            <el-tooltip placement="top" :content="t('dialog.group.actions.cancel_join_request_tooltip')"
                                :disabled="hideTooltips">
                                <span>
                                    <el-button type="default" icon="el-icon-close" circle style="margin-left: 5px"
                        ...
                        <template v-else-if="groupDialog.ref.myMember?.membershipStatus === 'invited'">
                            <el-tooltip placement="top" :content="t('dialog.group.actions.pending_request_tooltip')"
                                :disabled="hideTooltips">
                                <span>
                                    <el-button type="default" icon="el-icon-check" circle style="margin-left: 5px"
                                        @click="joinGroup(groupDialog.id)"></el-button>
                                </span>
                            </el-tooltip>
                        </template>
                        <template v-else>
                            <el-tooltip v-if="groupDialog.ref.joinState === 'request'" placement="top"
                                :content="t('dialog.group.actions.request_join_tooltip')" :disabled="hideTooltips">
                                <el-button type="default" icon="el-icon-message" circle style="margin-left: 5px"
                                    @click="joinGroup(groupDialog.id)"></el-button>
                            </el-tooltip>
                            <el-tooltip v-if="groupDialog.ref.joinState === 'invite'" placement="top"
                                :content="t('dialog.group.actions.invite_required_tooltip')" :disabled="hideTooltips">
                                <span>
                                    <el-button type="default" icon="el-icon-message" disabled circle
                                        style="margin-left: 5px"></el-button>
                                </span>
                            </el-tooltip>
                            <el-tooltip v-if="groupDialog.ref.joinState === 'open'" placement="top"
                                :content="t('dialog.group.actions.join_group_tooltip')" :disabled="hideTooltips">
                                <el-button type="default" icon="el-icon-check" circle style="margin-left: 5px"
                                    @click="joinGroup(groupDialog.id)"></el-button>
                            </el-tooltip>
                        </template>
                        <el-dropdown trigger="click" size="small" style="margin-left: 5px"
                            ...
                            <el-button :type="groupDialog.ref.membershipStatus === 'userblocked' ? 'danger' : 'default'"
                            ...
                            <el-dropdown-menu slot="dropdown">
                                ...
                                <template v-if="groupDialog.inGroup">
                                    <template v-if="groupDialog.ref.myMember">
                                        ...
                                        <el-dropdown-item :disabled="!hasGroupModerationPermission(groupDialog.ref)"
                                            icon="el-icon-s-operation" command="Moderation Tools">
                                        ...
                                        <template
                                            ...
                                            <el-dropdown-item icon="el-icon-view" command="Visibility Everyone" divided>
                                                <i v-if="groupDialog.ref.myMember.visibility === 'visible'"
                                                    class="el-icon-check"></i>
                                                {{ t('dialog.group.actions.visibility_everyone') }}
                                            </el-dropdown-item>
                                            <el-dropdown-item icon="el-icon-view" command="Visibility Friends">
                                                <i v-if="groupDialog.ref.myMember.visibility === 'friends'"
                                                    class="el-icon-check"></i>
                                                {{ t('dialog.group.actions.visibility_friends') }}
                                            </el-dropdown-item>
                                            <el-dropdown-item icon="el-icon-view" command="Visibility Hidden">
                                                <i v-if="groupDialog.ref.myMember.visibility === 'hidden'"
                                                    class="el-icon-check"></i>
                                                {{ t('dialog.group.actions.visibility_hidden') }}
                                            </el-dropdown-item>
                                        </template>
                                        <el-dropdown-item icon="el-icon-delete" command="Leave Group"
                                            style="color: #f56c6c" divided>
                                            {{ t('dialog.group.actions.leave') }}
                                        </el-dropdown-item>
                                    </template>
                                </template>
                                <template v-else>
                                    <el-dropdown-item v-if="groupDialog.ref.membershipStatus === 'userblocked'"
                                        icon="el-icon-circle-check" command="Unblock Group" style="color: #f56c6c"
                                        divided>
            ...
            <el-tabs v-model="groupDialogTabCurrentName" @tab-click="groupDialogTabClick">
                <el-tab-pane :label="t('dialog.group.info.header')">
                    <div class="group-banner-image-info">
                        <el-popover placement="right" width="500px" trigger="click">
                            <img slot="reference" v-lazy="groupDialog.ref.bannerUrl" class="x-link" style="
                                    flex: none;
                                    width: 100%;
                                    aspect-ratio: 6/1;
                                    object-fit: cover;
                                    border-radius: 4px;
                                " />
                            <img v-lazy="groupDialog.ref.bannerUrl" class="x-link" style="width: 854px; height: 480px"
                                @click="showFullscreenImageDialog(groupDialog.ref.bannerUrl)" />
                        </el-popover>
                    </div>
                    <div class="x-friend-list" style="max-height: none">
                        <span v-if="groupDialog.instances.length"
                        ...
                        <div v-for="room in groupDialog.instances" :key="room.tag" style="width: 100%">
                            <div style="margin: 5px 0">
                                ...
                                <InstanceInfo :location="room.tag" :instance="room.ref"
                                    :friendcount="room.friendCount" />
                            </div>
                            <div v-if="room.users.length" class="x-friend-list"
                        ...
                        <div class="x-friend-item" style="width: 100%; cursor: default">
                            <div class="detail">
                                ...
                                <div v-if="groupDialog.announcement.imageUrl"
                                    style="display: inline-block; margin-right: 5px">
                                    <el-popover placement="right" width="500px" trigger="click">
                                        <img slot="reference" v-lazy="groupDialog.announcement.imageUrl" class="x-link"
                                            style="
                                                flex: none;
                                                width: 60px;
                                                height: 60px;
                                                ...
                                                object-fit: cover;
                                            " />
                                        <img v-lazy="groupDialog.announcement.imageUrl" class="x-link"
                                            style="height: 500px"
                                ...
                                <br />
                                <div v-if="groupDialog.announcement.id" class="extra"
                                    ...
                                    <el-tooltip v-if="groupDialog.announcement.roleIds.length" placement="top">
                                        <template #content>
                                            ...
                                            <template v-for="roleId in groupDialog.announcement.roleIds">
                                                <template v-for="role in groupDialog.ref.roles"><span
                                                        v-if="role.id === roleId" :key="roleId + role.id"
                                                        v-text="role.name" /></template>
                                                <span :key="roleId" v-if="
                                    ...
                                    <el-tooltip placement="bottom">
                                        <template #content>
                                            <span>{{ t('dialog.group.posts.created_at') }}
                                            ...
                                            <template v-if="
                                                ...
                                                ">
                                                <br />
                                                <span>{{ t('dialog.group.posts.edited_at') }}
                                                    {{
                        ...
                        <div class="x-friend-item" style="cursor: default">
                            <div class="detail">
                                ...
                                <div v-if="groupDialog.ref.links && groupDialog.ref.links.length > 0"
                                    style="margin-top: 5px">
                                    <el-tooltip v-for="(link, index) in groupDialog.ref.links" v-if="link" :key="index">
                                        <template #content>
                                            <span v-text="link" />
                                        </template>
                                        <img :src="getFaviconUrl(link)" style="
                                                width: 16px;
                                                height: 16px;
                        ...
                        <div class="x-friend-item" style="width: 350px; cursor: default">
                            <div class="detail">
                                ...
                                <span class="extra">{{ groupDialog.ref.$url }}
                                    <el-tooltip placement="top" :content="t('dialog.group.info.url_tooltip')"
                                        :disabled="hideTooltips">
                                        <el-button type="default" size="mini" icon="el-icon-s-order" circle
                                            style="margin-left: 5px" @click="copyToClipboard(groupDialog.ref.$url)" />
                                    </el-tooltip></span>
                            </div>
                        </div>
                        <div class="x-friend-item" style="width: 350px; cursor: default">
                            <div class="detail">
                                ...
                                <span class="extra">{{ groupDialog.id }}
                                    <el-tooltip placement="top" :content="t('dialog.group.info.id_tooltip')"
                                        :disabled="hideTooltips">
                                        <el-button type="default" size="mini" icon="el-icon-s-order" circle
                                            style="margin-left: 5px" @click="copyToClipboard(groupDialog.id)" />
                                    </el-tooltip></span>
                            </div>
                        </div>
                        <div v-if="groupDialog.ref.membershipStatus === 'member'"
                ...
                <el-tab-pane :label="t('dialog.group.posts.header')" lazy>
                    <template v-if="groupDialog.visible">
                        <span style="margin-right: 10px">{{ t('dialog.group.posts.posts_count') }} {{
                            groupDialog.posts.length }}</span>
                        <el-input v-model="groupDialog.postsSearch" clearable size="mini"
                            :placeholder="t('dialog.group.posts.search_placeholder')"
                            style="width: 89%; margin-bottom: 10px" @input="updateGroupPostSearch" />
                        <div class="x-friend-list">
                            <div v-for="post in groupDialog.postsFiltered" :key="post.id" class="x-friend-item"
                                style="width: 100%; cursor: default">
                                <div class="detail">
                                    <span style="display: block" v-text="post.title" />
                                    <div v-if="post.imageUrl" style="display: inline-block; margin-right: 5px">
                                        <el-popover placement="right" width="500px" trigger="click">
                                            <img slot="reference" v-lazy="post.imageUrl" class="x-link" style="
                                                    flex: none;
                                                    width: 60px;
                                                    height: 60px;
                                                    border-radius: 4px;
                                                    object-fit: cover;
                                                " />
                                            <img v-lazy="post.imageUrl" class="x-link" style="height: 500px"
                                    ...
                                    <div v-if="post.authorId" class="extra" style="float: right; margin-left: 5px">
                                        <el-tooltip v-if="post.roleIds.length" placement="top">
                                            <template slot="content">
                                                ...
                                                <template v-for="roleId in post.roleIds">
                                                    <template v-for="role in groupDialog.ref.roles"><span
                                                            v-if="role.id === roleId" :key="role.id + roleId"
                                                            v-text="role.name" />
                                                    </template>
                                                    <template
                                                        v-if="post.roleIds.indexOf(roleId) < post.roleIds.length - 1"><span
                                                            :key="roleId">,&nbsp;</span></template>
                                                </template>
                                            </template>
                                            <i class="el-icon-view" style="margin-right: 5px" />
                                        </el-tooltip>
                                        ...
                                        <template
                                            ...
                                            <el-tooltip placement="top"
                                                ...
                                                :disabled="hideTooltips">
                                                <el-button type="text" icon="el-icon-delete" size="mini"
                                                    style="margin-left: 5px" @click="confirmDeleteGroupPost(post)" />
                                            </el-tooltip>
                                        </template>
                                    </div>
                                </div>
                ...
                <el-tab-pane :label="t('dialog.group.members.header')" lazy>
                    <template v-if="groupDialog.visible">
                        <span v-if="hasGroupPermission(groupDialog.ref, 'group-members-viewall')"
                        ...
                        <div style="margin-top: 10px">
                            <el-button type="default" size="mini" icon="el-icon-refresh"
                                :loading="isGroupMembersLoading" circle @click="loadAllGroupMembers" />
                            <el-button type="default" size="mini" icon="el-icon-download" circle
                                style="margin-left: 5px"
                                @click="downloadAndSaveJson(`${groupDialog.id}_members`, groupDialog.members)" />
                            <span v-if="groupDialog.memberSearch.length"
                            ...
                            <div v-if="hasGroupPermission(groupDialog.ref, 'group-members-manage')"
                                style="float: right">
                                ...
                                <el-dropdown trigger="click" size="small" style="margin-right: 5px"
                                    ...
                                    <el-dropdown-menu slot="dropdown">
                                        <el-dropdown-item v-for="item in groupDialogSortingOptions" :key="item.name"
                                            @click.native="setGroupMemberSortOrder(item)" v-text="t(item.name)" />
                                    </el-dropdown-menu>
                                </el-dropdown>
                                ...
                                <el-dropdown trigger="click" size="small" style="margin-right: 5px"
                                    ...
                                    <el-dropdown-menu slot="dropdown">
                                        <el-dropdown-item v-for="item in groupDialogFilterOptions" :key="item.name"
                                            @click.native="setGroupMemberFilter(item)" v-text="t(item.name)" />
                                        <el-dropdown-item v-for="item in groupDialog.ref.roles" v-if="!item.defaultRole"
                                            :key="item.name" @click.native="setGroupMemberFilter(item)"
                                            v-text="item.name" />
                                    </el-dropdown-menu>
                                </el-dropdown>
                            </div>
                            <el-input v-model="groupDialog.memberSearch" clearable size="mini"
                                :placeholder="t('dialog.group.members.search')"
                                style="margin-top: 10px; margin-bottom: 10px" @input="groupMembersSearch" />
                        </div>
                        <div v-if="groupDialog.memberSearch.length" v-loading="isGroupMembersLoading"
                            class="x-friend-list"
                            style="margin-top: 10px; overflow: auto; max-height: 250px; min-width: 130px">
                            <div v-for="user in groupDialog.memberSearchResults" :key="user.id"
                                class="x-friend-item x-friend-item-border" @click="showUserDialog(user.userId)">
                                <div class="avatar">
                                    <img v-lazy="userImage(user.user)" />
                                </div>
                                <div class="detail">
                                    <span class="name" :style="{ color: user.user.$userColour }"
                                        v-text="user.user.displayName" />
                                    <span class="extra">
                                        <template v-if="hasGroupPermission(groupDialog.ref, 'group-members-manage')">
                                            <el-tooltip v-if="user.isRepresenting" placement="top"
                                            ...
                                            <el-tooltip v-if="!user.isSubscribedToAnnouncements" placement="top"
                                        ...
                                        <template v-for="roleId in user.roleIds">
                                            <template v-for="role in groupDialog.ref.roles"><span
                                                    v-if="role.id === roleId" :key="role.id + roleId"
                                                    v-text="role.name" /></template><template
                                                v-if="user.roleIds.indexOf(roleId) < user.roleIds.length - 1"><span
                                                    :key="roleId">,&nbsp;</span></template>
                                        </template>
                                    </span>
                                </div>
                        ...
                        <ul v-else-if="groupDialog.members.length > 0" v-infinite-scroll="loadMoreGroupMembers"
                            class="infinite-list x-friend-list"
                            style="margin-top: 10px; overflow: auto; max-height: 250px; min-width: 130px">
                            <li v-for="user in groupDialog.members" :key="user.id"
                                ...
                                <div class="detail">
                                    ...
                                    <span class="extra">
                                        ...
                                        <template v-for="roleId in user.roleIds">
                                            <template v-for="role in groupDialog.ref.roles"><span
                                                    v-if="role.id === roleId" :key="roleId + role"
                                                    v-text="role.name" /></template><template
                                                v-if="user.roleIds.indexOf(roleId) < user.roleIds.length - 1"><span
                                                    :key="roleId">&nbsp;</span></template>
                                        </template>
                            ...
                            <div v-if="!isGroupMembersDone" v-loading="isGroupMembersLoading" class="x-friend-item"
                ...
                <el-tab-pane :label="t('dialog.group.gallery.header')" lazy>
                    <el-button type="default" size="mini" icon="el-icon-refresh" :loading="isGroupGalleryLoading" circle
                        @click="getGroupGalleries" />
                    <el-tabs v-model="groupDialogGalleryCurrentName" v-loading="isGroupGalleryLoading" type="card"
                        ...
                        <template v-for="(gallery, index) in groupDialog.ref.galleries">
                            <el-tab-pane :key="index">
                                <span slot="label">
                                    ...
                                    <i class="x-status-icon" style="margin-left: 5px"
                                ...
                                <el-carousel :interval="0" height="600px" style="margin-top: 10px">
                                    <el-carousel-item v-for="image in groupDialog.galleries[gallery.id]"
                                        :key="image.id">
                                        <el-popover placement="top" width="700px" trigger="click">
                                            <img slot="reference" v-lazy="image.imageUrl" class="x-link"
                                                style="width: 100%; height: 100%; object-fit: contain" />
                                            <img v-lazy="image.imageUrl" class="x-link" style="height: 700px"
                ...
                <el-tab-pane :label="t('dialog.group.json.header')" lazy>
                    <el-button type="default" size="mini" icon="el-icon-refresh" circle
                        @click="useVscodeLikeEditorToShowJSON ? refreshGroupDialogData() : refreshGroupDialogTreeData()" />
                    ...
                    <el-tree v-if="!useVscodeLikeEditorToShowJSON" :data="groupDialog.treeData"
                    ...
                    <MonacoEditor v-else :value="formatJSON(groupDialog.data)"
        ...
        <InviteGroupDialog />
        <PreviousInstancesGroupDialog :previous-instances-group-dialog.sync="previousInstancesGroupDialog"
