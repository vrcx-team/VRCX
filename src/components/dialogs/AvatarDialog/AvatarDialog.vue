<template>
    <safe-dialog ref="avatarDialogRef" class="x-dialog x-avatar-dialog" :visible.sync="avatarDialog.visible"
        :show-close="false" width="700px">
        <div v-loading="avatarDialog.loading">
            <div style="display: flex">
                <el-popover placement="right" width="500px" trigger="click">
                    <img slot="reference" v-lazy="avatarDialog.ref.thumbnailImageUrl" class="x-link"
                        style="flex: none; width: 160px; height: 120px; border-radius: 12px" />
                    <img v-lazy="avatarDialog.ref.imageUrl" class="x-link" style="width: 500px; height: 375px"
                ...
                <div style="flex: 1; display: flex; align-items: center; margin-left: 15px">
                    <div style="flex: 1">
                        ...
                        <div>
                            <el-tag v-if="avatarDialog.ref.releaseStatus === 'public'" type="success" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">{{ t('dialog.avatar.tags.public')
                                }}</el-tag>
                            <el-tag v-else type="danger" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">{{ t('dialog.avatar.tags.private')
                                }}</el-tag>
                            <el-tag v-if="avatarDialog.isPC" class="x-tag-platform-pc" type="info" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">PC
                                <span v-if="avatarDialog.platformInfo.pc" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.platformInfo.pc.performanceRating }}</span>
                                <span v-if="avatarDialog.bundleSizes['standalonewindows']" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.bundleSizes['standalonewindows'].fileSize }}</span>
                            </el-tag>
                            <el-tag v-if="avatarDialog.isQuest" class="x-tag-platform-quest" type="info" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">Android
                                <span v-if="avatarDialog.platformInfo.android" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.platformInfo.android.performanceRating }}</span>
                                <span v-if="avatarDialog.bundleSizes['android']" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.bundleSizes['android'].fileSize }}</span>
                            </el-tag>
                            <el-tag v-if="avatarDialog.isIos" class="x-tag-platform-ios" type="info" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">iOS
                                <span v-if="avatarDialog.platformInfo.ios" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.platformInfo.ios.performanceRating }}</span>
                                <span v-if="avatarDialog.bundleSizes['ios']" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.bundleSizes['ios'].fileSize }}</span>
                            </el-tag>
                            <el-tag v-if="avatarDialog.inCache" class="x-link" type="info" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px"
                                @click="openFolderGeneric(avatarDialog.cachePath)">
                                <span v-text="avatarDialog.cacheSize"></span>
                                &nbsp;{{ t('dialog.avatar.tags.cache') }}
                            </el-tag>
                            <el-tag v-if="avatarDialog.ref.styles?.primary || avatarDialog.ref.styles?.secondary"
                                type="info" effect="plain" size="mini" style="margin-right: 5px; margin-top: 5px">Styles
                                <span v-if="avatarDialog.ref.styles.primary" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.ref.styles.primary }}</span>
                                <span v-if="avatarDialog.ref.styles.secondary" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">{{
                                        avatarDialog.ref.styles.secondary }}</span>
                            </el-tag>
                            <el-tag v-if="avatarDialog.isQuestFallback" type="info" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">{{ t('dialog.avatar.tags.fallback')
                                }}</el-tag>
                            <el-tag v-if="avatarDialog.hasImposter" type="info" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">{{ t('dialog.avatar.tags.impostor') }}
                                <span v-if="avatarDialog.imposterVersion" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">v{{
                                        avatarDialog.imposterVersion }}</span>
                            </el-tag>
                            <el-tag v-if="avatarDialog.ref.unityPackageUrl" type="success" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">{{ t('dialog.avatar.tags.future_proofing')
                                }}</el-tag>
                            <div>
                                <template v-for="tag in avatarDialog.ref.tags">
                                    <el-tag v-if="tag.startsWith('content_')" :key="tag" effect="plain" size="mini"
                                        style="margin-right: 5px; margin-top: 5px">
                                    ...
                                    <el-tag v-if="tag.startsWith('author_tag_')" :key="tag" effect="plain" size="mini"
                                        style="margin-right: 5px; margin-top: 5px">
                                        <template>
                        ...
                        <div style="margin-top: 5px">
                            <span v-show="avatarDialog.ref.name !== avatarDialog.ref.description"
                                style="font-size: 12px" v-text="avatarDialog.ref.description"></span>
                        </div>
                    </div>
                    <div style="flex: none; margin-left: 10px">
                        <el-tooltip v-if="avatarDialog.inCache" placement="top"
                            :content="t('dialog.avatar.actions.delete_cache_tooltip')" :disabled="hideTooltips">
                        ...
                        <el-tooltip v-if="avatarDialog.isFavorite" placement="top"
                            :content="t('dialog.avatar.actions.favorite_tooltip')" :disabled="hideTooltips">
                        ...
                        <el-tooltip placement="top" :content="t('dialog.avatar.actions.select')"
                            :disabled="hideTooltips">
                            <el-button type="default" icon="el-icon-check" circle
                                :disabled="currentUser.currentAvatar === avatarDialog.id" style="margin-left: 5px"
                        ...
                        <el-dropdown trigger="click" size="small" style="margin-left: 5px"
                            ...
                            <el-button :type="avatarDialog.isBlocked ? 'danger' : 'default'" icon="el-icon-more"
                                circle></el-button>
                            <el-dropdown-menu>
                                ...
                                <el-dropdown-item v-if="avatarDialog.isBlocked" icon="el-icon-circle-check"
                                    command="Unblock Avatar" style="color: #f56c6c" divided>{{
                                ...
                                <el-dropdown-item v-if="/quest/.test(avatarDialog.ref.tags)" icon="el-icon-check"
                                    command="Select Fallback Avatar">{{ t('dialog.avatar.actions.select_fallback')
                                    }}</el-dropdown-item>
                                <el-dropdown-item v-if="avatarDialog.ref.authorId !== currentUser.id"
                                ...
                                <template v-if="avatarDialog.ref.authorId === currentUser.id">
                                    <el-dropdown-item v-if="avatarDialog.ref.releaseStatus === 'public'"
                                        icon="el-icon-user-solid" command="Make Private" divided>{{
                                    ...
                                    <el-dropdown-item v-if="avatarDialog.ref.unityPackageUrl" icon="el-icon-download"
                                        command="Download Unity Package">{{ t('dialog.avatar.actions.download_package')
                                        }}</el-dropdown-item>
                                    <el-dropdown-item v-if="avatarDialog.hasImposter" icon="el-icon-refresh"
                                        command="Regenerate Imposter" style="color: #f56c6c" divided>{{
                                            t('dialog.avatar.actions.regenerate_impostor') }}</el-dropdown-item>
                                    <el-dropdown-item v-if="avatarDialog.hasImposter" icon="el-icon-delete"
                                        command="Delete Imposter" style="color: #f56c6c">{{
            ...
            <el-tabs ref="avatarDialogTabsRef" @tab-click="avatarDialogTabClick">
                <el-tab-pane name="Info" :label="t('dialog.avatar.info.header')">
                    <div class="x-friend-list" style="max-height: unset">
                        <div v-if="avatarDialog.galleryImages.length || avatarDialog.ref.authorId === currentUser.id"
                            style="width: 100%">
                            <span class="name">{{ t('dialog.avatar.info.gallery') }}</span>
                            <input id="AvatarGalleryUploadButton" type="file" accept="image/*" style="display: none"
                                @change="onFileChangeAvatarGallery" />
                            <el-button v-if="avatarDialog.ref.authorId === currentUser.id"
                                :disabled="!!avatarDialog.galleryLoading" size="small" icon="el-icon-upload2"
                                style="margin-left: 5px" @click="displayAvatarGalleryUpload">{{
                                    t('dialog.screenshot_metadata.upload') }}</el-button>
                            <el-carousel v-if="avatarDialog.galleryImages.length" type="card" :autoplay="false"
                                height="200px">
                                <el-carousel-item v-for="imageUrl in avatarDialog.galleryImages" :key="imageUrl">
                                    ...
                                    <div v-if="avatarDialog.ref.authorId === currentUser.id"
                                        style="position: absolute; bottom: 5px; left: 33.3%">
                                        <el-button size="mini" icon="el-icon-back" circle class="x-link"
                                            style="margin-left: 0"
                                            @click.stop="reorderAvatarGalleryImage(imageUrl, -1)"></el-button>
                                        <el-button size="mini" icon="el-icon-right" circle class="x-link"
                                            style="margin-left: 0"
                                            @click.stop="reorderAvatarGalleryImage(imageUrl, 1)"></el-button>
                                        <el-button size="mini" icon="el-icon-delete" circle class="x-link"
                                            style="margin-left: 0"
                        ...
                        <div v-if="avatarDialog.ref.publishedListings?.length">
                            ...
                            <div v-for="listing in avatarDialog.ref.publishedListings" :key="listing.id"
                                class="x-friend-item" style="width: 100%; cursor: default">
                                <div class="avatar">
                                ...
                                <div class="detail">
                                    ...
                                    <span class="extra" style="text-overflow: ellipsis; text-wrap: auto"
                        ...
                        <div class="x-friend-item" style="width: 100%; cursor: default">
                            <div class="detail">
                                ...
                                <el-input v-model="memo" class="extra" size="mini" type="textarea" :rows="2"
                                    :autosize="{ minRows: 1, maxRows: 20 }"
                                    :placeholder="t('dialog.avatar.info.memo_placeholder')" resize="none"
                        ...
                        <div class="x-friend-item" style="width: 100%; cursor: default">
                            <div class="detail">
                                ...
                                <span class="extra">{{ avatarDialog.id
                        ...
                        <div class="x-friend-item" style="cursor: default">
                            <div class="detail">
                                ...
                                <span v-if="avatarDialog.ref.version !== 0" class="extra"
                ...
                <el-tab-pane name="JSON" :label="t('dialog.avatar.json.header')" style="max-height: 50vh" lazy>
                    <el-button type="default" size="mini" icon="el-icon-refresh" circle
                        @click="useVscodeLikeEditorToShowJSON ? refreshAvatarDialogData() : refreshAvatarDialogTreeData()"></el-button>
                    ...
                    <el-tree v-if="!useVscodeLikeEditorToShowJSON && Object.keys(avatarDialog.fileAnalysis).length > 0"
                        :data="avatarDialog.fileAnalysis" style="margin-top: 5px; font-size: 12px">
                    ...
                    <MonacoEditor v-else-if="Object.keys(avatarDialog.fileAnalysis).length > 0"
                        style="margin-top: 5px; font-size: 12px; min-height: 50vh;"
                        :value="formatJSON(avatarDialog.fileAnalysis)" />
                    <el-tree v-if="!useVscodeLikeEditorToShowJSON" :data="treeData"
                    ...
                    <MonacoEditor v-else style="margin-top: 5px; font-size: 12px; min-height: 50vh;"
        ...
        <ChangeAvatarImageDialog :change-avatar-image-dialog-visible.sync="changeAvatarImageDialogVisible"
            :previous-images-file-id="previousImagesFileId" @refresh="displayPreviousImages" />
