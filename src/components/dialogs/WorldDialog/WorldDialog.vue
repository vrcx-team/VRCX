<template>
    <safe-dialog ref="worldDialogRef" class="x-dialog x-world-dialog" :visible.sync="isDialogVisible"
        :show-close="false" width="770px">
        <div v-loading="worldDialog.loading">
            <div style="display: flex">
                <el-popover placement="right" width="500px" trigger="click">
                    <img slot="reference" :src="worldDialog.ref.thumbnailImageUrl" class="x-link"
                        style="flex: none; width: 160px; height: 120px; border-radius: 12px" />
                    <img v-lazy="worldDialog.ref.imageUrl" class="x-link" style="width: 500px; height: 375px"
                    ...
                    <div style="flex: 1">
                        <div>
                            <el-popover placement="top" trigger="click">
                                <span slot="reference" class="dialog-title" style="margin-right: 5px; cursor: pointer"
                                    ...
                                    " class="el-icon-s-home" style="margin-right: 5px" />
                        ...
                        <div style="margin-top: 5px">
                            <span class="x-link x-grey" style="font-family: monospace"
                                @click="showUserDialog(worldDialog.ref.authorId)" v-text="worldDialog.ref.authorName" />
                        </div>
                        <div>
                            <el-tag v-if="worldDialog.ref.$isLabs" type="primary" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.world.tags.labs') }}
                            </el-tag>
                            <el-tag v-else-if="worldDialog.ref.releaseStatus === 'public'" type="success" effect="plain"
                            ...
                            <el-tag v-if="worldDialog.isPC" class="x-tag-platform-pc" type="info" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                PC<span v-if="worldDialog.bundleSizes['standalonewindows']" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">
                                    {{ worldDialog.bundleSizes['standalonewindows'].fileSize }}
                                </span>
                            </el-tag>

                            <el-tag v-if="worldDialog.isQuest" class="x-tag-platform-quest" type="info" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                Android<span v-if="worldDialog.bundleSizes['android']" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">
                                    {{ worldDialog.bundleSizes['android'].fileSize }}
                                </span>
                            </el-tag>

                            <el-tag v-if="worldDialog.isIos" class="x-tag-platform-ios" type="info" effect="plain"
                                size="mini" style="margin-right: 5px; margin-top: 5px">
                                iOS<span v-if="worldDialog.bundleSizes['ios']" class="x-grey"
                                    style="margin-left: 5px; border-left: inherit; padding-left: 5px">
                                    {{ worldDialog.bundleSizes['ios'].fileSize }}
                                </span>
                            </el-tag>

                            <el-tag v-if="worldDialog.avatarScalingDisabled" type="warning" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.world.tags.avatar_scaling_disabled') }}
                            </el-tag>
                            <el-tag v-if="worldDialog.focusViewDisabled" type="warning" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.world.tags.focus_view_disabled') }}
                            </el-tag>
                            <el-tag v-if="worldDialog.ref.unityPackageUrl" type="success" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px">
                                {{ t('dialog.world.tags.future_proofing') }}
                            </el-tag>
                            <el-tag v-if="worldDialog.inCache" class="x-link" type="info" effect="plain" size="mini"
                                style="margin-right: 5px; margin-top: 5px"
                        ...
                        <div style="margin-top: 5px">
                            <span v-show="worldDialog.ref.name !== worldDialog.ref.description"
                                style="font-size: 12px">{{
                                    worldDialog.ref.description }}</span>
                        </div>
                    </div>
                    <div style="flex: none; margin-left: 10px">
                        <el-tooltip v-if="worldDialog.inCache" placement="top"
                            :content="t('dialog.world.actions.delete_cache_tooltip')" :disabled="hideTooltips">
                            <el-button icon="el-icon-delete" circle :disabled="isGameRunning && worldDialog.cacheLocked"
                                @click="deleteVRChatCache(worldDialog.ref)" />
                        </el-tooltip>
                        <el-tooltip placement="top" :content="t('dialog.world.actions.favorites_tooltip')"
                            :disabled="hideTooltips">
                            <el-button :type="worldDialog.isFavorite ? 'warning' : 'default'"
                                :icon="worldDialog.isFavorite ? 'el-icon-star-on' : 'el-icon-star-off'" circle
                                style="margin-left: 5px" @click="worldDialogCommand('Add Favorite')" />
                        </el-tooltip>
                        <el-dropdown trigger="click" size="small" style="margin-left: 5px"
                            ...
                            <el-dropdown-menu slot="dropdown">
                                ...
                                <el-dropdown-item v-if="
                                    currentUser.$homeLocation &&
                                    currentUser.$homeLocation.worldId === worldDialog.id
                                " icon="el-icon-magic-stick" command="Reset Home" divided>
                                ...
                                <template v-if="currentUser.id !== worldDialog.ref.authorId">
                                    ...
                                    <el-dropdown-item :disabled="!worldDialog.hasPersistData" icon="el-icon-upload"
                                ...
                                <template v-else>
                                    ...
                                    <el-dropdown-item v-if="worldDialog.ref.unityPackageUrl" icon="el-icon-download"
                                    ...
                                    " icon="el-icon-view" command="Unpublish" divided>
            ...
            <el-tabs ref="worldDialogTabsRef" @tab-click="worldDialogTabClick">
                    ...
                    <div v-for="room in worldDialog.rooms" :key="room.id">
                        <template
                            ...
                            <div style="margin: 5px 0">
                                <LocationWorld :locationobject="room.$location" :currentuserid="currentUser.id"
                                    :worlddialogshortname="worldDialog.$location.shortName" />
                                <Launch :location="room.tag" style="margin-left: 5px" />
                                <InviteYourself :location="room.$location.tag" :shortname="room.$location.shortName"
                                    style="margin-left: 5px" />
                                <el-tooltip placement="top" :content="t('dialog.world.instances.refresh_instance_info')"
                                    :disabled="hideTooltips">
                                    <el-button size="mini" icon="el-icon-refresh" style="margin-left: 5px" circle
                                        @click="refreshInstancePlayerCount(room.tag)" />
                                </el-tooltip>
                                <el-tooltip v-if="instanceJoinHistory.get(room.$location.tag)" placement="top"
                                    :content="t('dialog.previous_instances.info')" :disabled="hideTooltips">
                                    <el-button size="mini" icon="el-icon-s-data" style="margin-left: 5px" plain circle
                                ...
                                <div v-if="room.$location.userId || room.users.length" class="x-friend-list"
                                    style="margin: 10px 0; max-height: unset">
                                    <div v-if="room.$location.userId" class="x-friend-item x-friend-item-border"
                                        ...
                                        <template v-if="room.$location.user">
                                            ...
                                            <div class="detail">
                                                <span class="name" :style="{ color: room.$location.user.$userColour }"
                    ...
                    <div class="x-friend-list" style="max-height: none">
                        <div class="x-friend-item" style="width: 100%; cursor: default">
                            <div class="detail">
                                ...
                                <el-input v-model="memo" class="extra" type="textarea" :rows="2"
                                    :autosize="{ minRows: 1, maxRows: 20 }"
                                    :placeholder="t('dialog.world.info.memo_placeholder')" size="mini" resize="none"
                        ...
                        <div v-if="worldDialog.ref.previewYoutubeId" class="x-friend-item" style="width: 350px" @click="
                        ...
                        <div class="x-friend-item" style="cursor: default">
                            <div class="detail">
                                ...
                                <span class="extra">
                                    ...
                                    }}<span v-if="worldDialog.ref?.favorites > 0 && worldDialog.ref?.visits > 0"
                                        class="extra">
                                        ({{ favoriteRate }}%)
                                    </span>
                                </span>
                        ...
                        <div v-if="worldDialog.ref.labsPublicationDate !== 'none'" class="x-friend-item"
                        ...
                        <div v-if="worldDialog.ref.publicationDate !== 'none'" class="x-friend-item"
                            style="cursor: default">
                            <div class="detail">
                            ...
                            <div class="detail">
                                <span class="name">
                                    ...
                                    <el-tooltip v-if="!hideTooltips" placement="top" style="margin-left: 5px"
                                        :content="t('dialog.world.info.accuracy_notice')"><i
                        ...
                        <div class="x-friend-item" style="cursor: default">
                            <div class="detail">
                                <span class="name">{{ t('dialog.world.info.time_spent') }}
                                    <el-tooltip v-if="!hideTooltips" placement="top" style="margin-left: 5px"
                ...
                <el-tab-pane name="JSON" :label="t('dialog.world.json.header')" style="max-height: 50vh" lazy>
                    ...
                    <el-tree v-if="!useVscodeLikeEditorToShowJSON && Object.keys(worldDialog.fileAnalysis).length > 0"
                        :data="worldDialog.fileAnalysis" style="margin-top: 5px; font-size: 12px">
                    ...
                    <MonacoEditor v-else-if="Object.keys(worldDialog.fileAnalysis).length > 0"
                        style="margin-top: 5px; font-size: 12px; min-height: 50vh;"
                        :value="formatJSON(worldDialog.fileAnalysis)" />
                    <el-tree v-if="Object.keys(treeData).length > 0" :data="treeData"
                    ...
                    <MonacoEditor v-else-if="Object.keys(data).length > 0"
                        style="margin-top: 5px; font-size: 12px; min-height: 50vh;" :value="formatJSON(data)" />
                </el-tab-pane>
            </el-tabs>
        </div>
        ...
        <SetWorldTagsDialog :is-set-world-tags-dialog-visible.sync="isSetWorldTagsDialogVisible"
            :old-tags="worldDialog.ref?.tags" :world-id="worldDialog.id"
        ...
        <NewInstanceDialog :new-instance-dialog-location-tag="newInstanceDialogLocationTag"
            :last-location="lastLocation" />
        <ChangeWorldImageDialog :change-world-image-dialog-visible.sync="changeWorldImageDialogVisible"
            :previous-images-file-id="previousImagesFileId" :world-dialog="worldDialog"
...
const { hideTooltips, isAgeGatedInstancesVisible, useVscodeLikeEditorToShowJSON } = storeToRefs(useAppearanceSettingsStore());
